...
-- <> is the ANSI-SQL NOT equal to operator
SELECT city_name, state_abbreviation 
FROM city
WHERE state_abbreviation <> 'CA';
...
-- The names of all cities that contain a letter 'a' somewhere in them
SELECT city_name
FROM city
WHERE city_name LIKE '%a%';

-- ILIKE is a case insensitive comparison that is PostgreSQL specific
SELECT city_name
FROM city
